...
paste('The naive model produces MSE',mse_naive,'. Ridge Produces an MSE of ', mse.ridge)
```
...
ones_column = matrix(1, nrow=nrow(train_data_x), ncol=1)
train_data_x1 = cbind(ones_column, train_data_x)


#n.tmp is number of observation
#p.tmp is the number of variables, features, etc

solve(t(train_data_x1) %*% train_data_x1 + nrow(train_data_x1) * bestlambda * diag(ncol(train_data_x1))) %*% t(train_data_x1) %*% train_data_y


drop(solve(crossprod(train_data_x1) + diag( nrow(train_data_x1) * bestlambda, ncol(train_data_x1)), crossprod(train_data_x1, train_data_y)))
