...
data = select(data, -c(6, 8,9, 12,  13))
...
for (j in 1:7) {
  ...
  train_data_x = as.matrix(train_data[,-9]) # double check this indexes loglatestprice
  train_data_y = as.matrix(train_data[, 9])
  test_data_x = as.matrix(test_data[,-9])
  test_data_y = as.matrix(test_data[, 9])
...
for (j in 1:7) {
  ...
  train_data_x = as.matrix(train_data[,-9])
  train_data_y = as.matrix(train_data[, 9])
  test_data_x = as.matrix(test_data[,-9])
  test_data_y = as.matrix(test_data[, 9])
...
#lowest mean squared error was Ridge, so we will run this over entire training dataset
  ...
  train_data_x = as.matrix(train[,-9])
  train_data_y = as.matrix(train[, 9])
  test_data_x = as.matrix(test[,-9])
  test_data_y = as.matrix(test[, 9])
