...
cv_errors = sapply(grid, function(lambda) {
  model = lmridge(train_data_y ~ ., data = train_data_x, K = lambda)
  pred = predict(model, newdata = test_data_x)  # Predicting on the test set
  mean((test_data_y - pred)^2)  # Calculating MSPE
})

bestlambda = grid[which.min(cv_errors)]

# Plotting MSPE vs. lambda
plot(log10(grid), cv_errors, type = "b", main = "MSPE of Ridge with Respective Lambdas",
     xlab = "Log10(Lambda)", ylab = "MSPE")

ridge_model = lmridge(train_data_y ~ ., data = train_data_x, K = bestlambda)

ridge_coef = coef(ridge_model)

ridge_predict = predict(ridge_model, newdata = test_data_x)
