...
result_matrix = cbind(ridge.coef@Dimnames[[1]], ridge.coef@x)

lambda = 10
Y = as.matrix(train[, 6])
X = cbind(ones_column, as.matrix(train[,-6]))
lambdaI = diag(lambda, ncol(X))


betahat = solve(t(X) %*% X + lambdaI) %*% t(X) %*% Y
H = X %*% solve(t(X) %*% X + lambdaI) %*% t(X)
v = nrow(X) -sum(diag(H))
sigmahat = (t(Y - X %*% betahat) %*% (Y - X %*% betahat)) / v

varbetahat = solve(t(X) %*% X + lambdaI) %*% t(X) %*% X %*% solve(t(X) %*% X + lambdaI)
