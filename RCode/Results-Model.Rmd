...
```{r}
thresholds = seq(1.0e-01, 4.0e-01, length.out = 50)  
...
for (i in 1:length(thresholds)) {
  ...
  #remove zero paramters and make var to count number of non-zero coeffs 
  ...
  if (length(selected_predictors) > 0) {
    ...
    mse_values[i] = NA  # No predictors selected, threshold choosen so this shouldn't occur
  }
}
...
paste('Our coefficients form a linear model with MSE', mse_values[15], 'and the naive model produces MSE',mse_naive,'. We may thus conclude our reduced model is better at predicting house prices.')
```
...
coef_thresholded = ridge.coef
coef_thresholded[abs(coef_thresholded) < thresholds[15]] = 0
